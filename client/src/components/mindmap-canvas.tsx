...
function PictureBoard({ data, onNodeClick, editable }: {
  ...
  const cols = Math.min(4, branches.length) || 1;

  return (
    <div style={{ background: "#fffef5", borderRadius: 14, overflow: "hidden", fontFamily: "system-ui, sans-serif" }}>
      {/* Header */}
      <div
        style={{ background: "#7c3aed", padding: "16px 20px", display: "flex", alignItems: "center", gap: 14, cursor: editable ? "pointer" : "default" }}
        onClick={() => onNodeClick?.("central", data.centralTopic || data.title)}
      >
        {data.centralImage && (
          <img
            src={data.centralImage}
            alt=""
            style={{ width: 58, height: 58, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(255,255,255,0.45)", flexShrink: 0 }}
            onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }}
          />
        )}
        <div>
          <div style={{ color: "#fff", fontSize: 22, fontWeight: 700, lineHeight: 1.2 }}>{data.centralTopic || data.title}</div>
          <div style={{ color: "rgba(255,255,255,0.72)", fontSize: 13, marginTop: 3 }}>Vocabulary Picture Board</div>
        </div>
      </div>

      {/* Card grid */}
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 16, padding: 16 }}>
        {branches.map((b, i) => {
          const color = b.color || COLORS[i % COLORS.length];
          return (
            <div
              key={i}
              style={{ borderRadius: 14, border: `3px solid ${color}`, background: "#fff", overflow: "hidden", boxShadow: "2px 3px 10px rgba(0,0,0,0.09)", cursor: editable ? "pointer" : "default" }}
              onClick={() => onNodeClick?.(`branch-${i}`, b.label)}
            >
              {b.image ? (
                <img
                  src={b.image}
              ...
              <div
                style={{
                  width: "100%", aspectRatio: "4/3", background: `${color}22`,
                  display: b.image ? "none" : "flex",
                  alignItems: "center", justifyContent: "center",
                  fontSize: 52, color, fontWeight: 700,
                }}
              >
                {b.label.charAt(0)}
              </div>
              <div style={{ background: color, padding: "9px 6px", textAlign: "center", fontWeight: 700, fontSize: 14, color: "#fff", letterSpacing: "0.01em" }}>
                {b.label}
              </div>
            </div>
          );
